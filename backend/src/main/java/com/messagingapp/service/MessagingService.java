...
import com.messagingapp.repository.*;
...
public class MessagingService {
  ...
  public MessagingService(UserRepository u, ConversationRepository c, MessageRepository m) {
    users = u;
    conversations = c;
    messages = m;
  }

  public List<User> contacts(String me) {
    return users.findAll().stream().filter(u -> !u.id().equals(me)).toList();
  }

  public Conversation direct(String me, String other) {
    if (me.equals(other) || !users.existsById(other))
      throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Valid recipient required");
    var ids = List.of(me, other).stream().sorted().toList();
    var key = String.join(":", ids);
    return conversations
        .findByParticipantKey(key)
  ...
  public Message send(String me, String id, String content) {
    member(me, id);
    if (content == null || content.isBlank() || content.length() > 2000)
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "Message must be 1-2000 characters");
