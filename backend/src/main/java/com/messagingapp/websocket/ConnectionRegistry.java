...
public class ConnectionRegistry {
    ...
    public void remove(String userId, WebSocketSession session) {
        sessions.getOrDefault(userId, Set.of()).remove(session);
    }

    public void send(String userId, String event) throws IOException {
        for (WebSocketSession session : sessions.getOrDefault(userId, Set.of()))
            if (session.isOpen()) session.sendMessage(new TextMessage(event));
    }

    public void broadcast(String event) throws IOException {
